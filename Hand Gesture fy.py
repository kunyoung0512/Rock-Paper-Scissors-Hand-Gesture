...
max_num_hands = 1
gesture = {
    0:'fist', 1:'one', 2:'yeah', 3:'three', 4:'four', 5:'five',
...
cap = cv2.VideoCapture(0) #adjust 0,1,2 according to your webcam
...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks is not None:
        for res in result.multi_hand_landmarks:
            ...
            #Other gestures
            if idx == 11:
                x1, y1 = tuple((joint.min(axis=0)[:2]*[img.shape[1],img.shape[0]]).astype(int))
                x2, y2 = tuple((joint.max(axis=0)[:2]*[img.shape[1],img.shape[0]]).astype(int))\
